...
public class ComprarTest {
    ...
    public void testPublicar() throws InterruptedException {
        ...
        driver.manage().timeouts().implicitlyWait(Duration.ofMillis(5000));
        driver.findElement(By.xpath("//*[@id=\"email\"]")).sendKeys("rsol777ddd@gmail.com");
        ...
        //Comprobar resultado
        WebElement message = driver.findElement(By.id("step-section-primary-header"));
        String value = message.getText();
        if (value.equals("Pago")) {
            test.log(Status.PASS, "Se encontró el encabezado 'Pago'");
        } else if (value.equals("Contacto")) {
            test.log(Status.WARNING, "No avanzo a pago, sigue en contacto.");
        } else {
            test.log(Status.FAIL, "El encabezado esperado no fue encontrado.");
        }
        assertEquals("Pago", value);
    }
